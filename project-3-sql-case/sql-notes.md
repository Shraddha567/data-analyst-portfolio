...
Step 4: Create a User 'shraddha'@'localhost'
...
CREATE USER 'shraddha'@'localhost' IDENTIFIED BY 'password';
GRANT ALL PRIVILEGES ON *.* TO 'shraddha'@'localhost' WITH GRANT OPTION;
FLUSH PRIVILEGES;
EXIT;
...
2) Each table is a file inside that folder.
3) The rows in the table are like the content inside each file.

* Excel analogy:
1) A database is like an Excel workbook.
2) Each table is a separate sheet inside that workbook.
...
Now we’ll create a simple users table: This table will store basic user info.
...
values, useful for financial data. The first number is
the total number of digits, and the second is the
...
*Renaming a Table
To rename an existing table: -- RENAME TABLE users TO customers;
To rename it back: -- RENAME TABLE customers TO users;

* Altering a Table
We can use ALTER TABLE to modify an existing table.
Add a Column
Drop a Column
...
UPDATE users
SET gender = 'Other';
This updates every row in the table. Be very careful when omitting the WHERE
clause.
